...
| 📊 **市场情绪分析师** | 综合情绪量化 | 多维度情绪评分、资金流向、波动率分析 |
...
市场情绪分析师是Factor AI的核心创新功能，通过整合多维度数据源，提供量化的市场情绪评估。

### 🎯 核心功能

- **综合情绪评分** - 提供0-100的量化评分，直观反映市场情绪状态
- **多维度分析** - 整合新闻、资金流向、波动率、技术动量、社交媒体等多个维度
- **市场差异化** - 针对美股、A股、港股提供差异化的情绪指标
- **智能降级** - 数据源不可用时自动降级，确保服务稳定性
...
| 指标 | 权重 | 数据源 | 说明 |
|------|------|--------|------|
| 📰 新闻情绪 | 25% | Google News | 分析新闻标题和内容的情绪倾向 |
| 📊 价格动量 | 20% | Yahoo Finance / AKShare | 基于价格走势计算动量评分 |
| 📈 成交量分析 | 包含在技术动量中 | Yahoo Finance / AKShare | 分析成交量变化趋势 |

#### 美股增强层指标

| 指标 | 权重 | 数据源 | 说明 |
|------|------|--------|------|
| 😱 VIX恐慌指数 | 20% | Yahoo Finance | 芝加哥期权交易所波动率指数 |
| 💬 Reddit情绪 | 10% | Reddit API | 社交媒体投资者讨论情绪 |

#### A股增强层指标

| 指标 | 权重 | 数据源 | 说明 |
|------|------|--------|------|
| 💰 北向资金 | 25% | AKShare | 沪深港通资金流向 |
| 📊 融资融券 | 包含在资金流向中 | TuShare | 散户情绪指标 |
| 📉 波动率 | 20% | 计算得出 | 历史波动率百分位 |

#### 港股增强层指标

| 指标 | 权重 | 数据源 | 说明 |
|------|------|--------|------|
| 💰 南向资金 | 25% | AKShare | 港股通资金流向 |
...
```bash
# 情绪分析缓存配置
SENTIMENT_VIX_CACHE_TTL=300              # VIX缓存时长（秒）
SENTIMENT_NEWS_CACHE_TTL=1800            # 新闻缓存时长（秒）
SENTIMENT_NORTHBOUND_CACHE_TTL=3600      # 北向资金缓存时长（秒）

# 性能配置
SENTIMENT_ANALYSIS_TIMEOUT=10            # 分析超时时间（秒）
SENTIMENT_MAX_WORKERS=4                  # 最大并发线程数
```
...
| 评分范围 | 情绪等级 | 市场状态 | 投资建议 |
|----------|----------|----------|----------|
| 80-100 | 极度乐观 | 市场过热 | 警惕反转风险 |
| 60-79 | 乐观 | 市场向好 | 可适度参与 |
| 40-59 | 中性 | 震荡整理 | 观望为主 |
| 20-39 | 悲观 | 市场低迷 | 寻找机会 |
| 0-19 | 极度恐慌 | 市场恐慌 | 反向指标机会 |

### 🎯 使用场景

- **市场择时** - 识别市场极端情绪，把握反转机会
- **风险管理** - 评估市场整体风险水平
- **投资决策** - 结合基本面和技术面，做出综合判断
- **情绪追踪** - 跟踪市场情绪变化趋势
