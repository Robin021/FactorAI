...
def create_fixed_analysis_function():
    """创建修复后的分析函数"""
...
if __name__ == "__main__":
    print("🔧 生成修复后的分析函数...")
    fixed_code = create_fixed_analysis_function()
    
    with open("fixed_analysis_function.py", "w", encoding="utf-8") as f:
        f.write(fixed_code)
    
    print("✅ 修复后的函数已保存到 fixed_analysis_function.py")
    print("\n🎯 修复要点:")
    print("1. 消除了重复的 run_stock_analysis 函数定义")
    print("2. 直接在 analysis_worker 中执行分析逻辑")
    print("3. 统一了分析师配置为 ['market', 'fundamentals', 'social']")
    print("4. 简化了进度回调逻辑")
    print("5. 确保每个分析师只执行一次")
    
    print("\n📋 手动替换步骤:")
    print("1. 备份当前的 backend/tradingagents_server.py")
    print("2. 找到 start_real_analysis 函数")
    print("3. 用 fixed_analysis_function.py 中的内容替换整个函数")
    print("4. 重启服务器测试")