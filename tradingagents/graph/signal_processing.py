...
如果某些信息在报告中没有明确提及，请使用合理的默认值。""",
        ...
        try:
            ...
            if json_match:
                ...
                if target_price is None or target_price == "null" or target_price == "":
                    ...
                    # 增强的价格匹配模式
                    # 仅匹配明确的目标价格或带货币单位的数值，避免将百分比误识别为价格
                    price_patterns = [
                        r'目标价[位格]?[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',  # 目标价位: 45.50（不带百分号）
                        r'目标价格[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',      # 目标价格: 45.50
                        r'\*\*目标价[位格]?\*\*[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',  # **目标价位**: 45.50
                        r'[¥\$](\d+(?:\.\d+)?)',                                   # ¥45.50 或 $190
                        r'(\d+(?:\.\d+)?)元',                                        # 45.50元
                        r'(\d+(?:\.\d+)?)美元',                                      # 190美元
                        r'(\d+(?:\.\d+)?)\s*[¥\$]',                                 # 45.50¥
    ...
    def _extract_simple_decision(self, text: str) -> dict:
        ...
        # 尝试提取目标价格（使用增强的模式）
        target_price = None
        # 更严格的模式：避免把百分比当成价格；仅接受明确“目标价/目标价格”或带货币单位
        price_patterns = [
            r'目标价[位格]?[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',
            r'目标价格[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',
            r'\*\*目标价[位格]?\*\*[：:]?\s*[¥\$]?(\d+(?:\.\d+)?)(?!\s*%)',
